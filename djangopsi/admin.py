    ...
    for i in range(first_year, last_year + 1):
        ...
        for j in range(start, finish + 1):
            ...
            if mark not in marks:
                marks.append(mark)
                month_reports = reports.filter(
                    created__year__gte=i,
                ...
                overall_mobile_scores.append(
                    month_reports.filter(strategy="mobile").aggregate(Avg("score"))[
                        "score__avg"
                    ]
                )
                overall_desktop_scores.append(
                    month_reports.filter(strategy="desktop").aggregate(Avg("score"))[
                        "score__avg"
                    ]
                )
...
class EnvironmentAdmin(admin.ModelAdmin):
    ...
    report_group_dashboard_template = (
        "admin/djangopsi/environment/report_group_dashboard.html"
    )
    ...
    def dashboard(self, request, id):
        ...
        reports = Report.objects.filter(url__environment_id=environment.id).order_by(
            "created"
        )

        marks, overall_desktop_scores, overall_mobile_scores = get_marks_and_scores(
            reports
        )
    ...
    def url_dashboard(self, request, id, url_id):
        ...
        return render(
            request,
            self.url_dashboard_template,
            {
                "title": "Page Speed Dashboard: %s - %s"
                % (environment.name, path.name),
                "environment": environment,
                "path": path,
                "marks": json.dumps(marks),
                "overall_mobile_scores": json.dumps(overall_mobile_scores),
    ...
    def report_group_dashboard(self, request, id, report_group_id):
        ...
        return render(
            request,
            self.report_group_dashboard_template,
            {
                "title": "Page Speed Dashboard: %s environment - Report Group #%s"
                % (environment.name, report_group.id),
                "environment": environment,
                "paths": paths,
