...
class Command(BaseCommand):
    ...
    def _safety_check(self):
        ...
        if not (
            settings.PSI_ENVS["production"]
            or settings.PSI_ENVS["production"]["base_url"]
        ):
    ...
    def handle(self, *args, **options):
        ...
        psi_service = build(
            ...
            version="v5",
            developerKey=PSI_GOOGLE_API_DEV_KEY,
        ...
        if options["strategy"] != "all":
            url_reports = check_urls_in_pagespeed(
                psi_service, urls_to_check, analysis_base_url, options["strategy"]
            )
        else:
            self.stdout.write(self.style.SUCCESS("Generating mobile reports..."))
            url_reports += check_urls_in_pagespeed(
                psi_service, urls_to_check, analysis_base_url, "mobile"
            )
        ...
        if options["keep"]:
            ...
            for report in url_reports:
                url, _ = env.urls.get_or_create(
                    name=report["url"]["name"], path=report["url"]["path"]
                )
                url.reports.create(
