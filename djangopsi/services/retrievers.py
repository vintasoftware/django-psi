    ...
    for pattern in urlpatterns:
        if isinstance(pattern, URLResolver):
            ...
            get_all_project_urls_to_check(
                pattern.url_patterns, url_list, namespace=pattern.namespace
            )
        elif isinstance(pattern, URLPattern):
            callback_func = pattern.callback
            if (
                hasattr(callback_func, "view_class")
                and hasattr(callback_func.view_class, "is_psi_checked")
                and callback_func.view_class.is_psi_checked
            ):
                pattern_name = (
...
def check_urls_in_pagespeed(psi_service, urls, base_url, strategy):
    ...
    for i in range(len(urls)):
        report = run_pagespeed_analysis(
            psi_service, base_url + urls[i]["path"], strategy
        )
        if report:
